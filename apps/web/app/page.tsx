...
import { actionQueue, loadAutomationSettings, todaysActivity } from "@depinfin/db";
...
/**
 * Section 9 item 1 and item 2, on one screen.
 *
 * The queue is everything in `replied` status, newest first, and it is the
 * only thing that matters most mornings. Below it, what the dispatcher did
 * today with the reason it gives, rendered as it comes out of the pipeline.
 * Those reasons are already written for a person. Mapping them back to codes
...
export default async function QueuePage() {
  ...
  const skipped = today.filter((row) => row.action !== "sent");
  ...
  return (
    <>
      ...
      <div className="row" style={{ marginBottom: 10 }}>
        ...
        <span className="tag">{skipped.length} skipped or blocked</span>
          ...
          <tbody>
            {today.map((row, index) => (
              <tr key={`${row.occurred_at.toISOString()}-${index}`}>
                ...
                <td>
                  <span className={`tag ${row.action === "sent" ? "good" : row.action === "blocked" ? "bad" : ""}`}>
                    {row.action}
                  </span>
                </td>
                <td>
                ...
                {/* Rendered exactly as the pipeline wrote it. Section 9. */}
